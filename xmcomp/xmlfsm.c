...
int xmlfsm_recover(BufferPtr *buffer, BufferPtr *real_buffer) {
	// At this stage we are assuming there's at least 1 complete stanza in the buffer
/*
 *
 * BEWARE OF STRINGS
 *
// 2) go through with xmlfsm_skip_node algorithm. If never less than 0, flow is OK
// 3) whenever less than zero, discard the region scanned and go to 2

	char *tag_closing_pos = 0, *orig_start = buffer->data;
	int nesting_level = 0;
	char quote_char = 0;
	char *current_end = (real_buffer && buffer->data > buffer->end) ? real_buffer->end : buffer->end;
